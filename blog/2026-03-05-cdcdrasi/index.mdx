...
[Change Data Capture (CDC)](https://learn.microsoft.com/azure/postgresql/flexible-server/concepts-logical?WT.mc_id=AZ-MVP-5004796) flips this model. Instead of asking the database whether something has changed, the database tells you when it does.
...
Drasi sits on top of this CDC stream and runs [continuous queries](https://drasi.io/concepts/continuous-queries/) - written in Cypher - that evaluate incoming changes against patterns you define. When a pattern matches, Drasi fires a reaction _(in my case, an HTTP callback to an API)_.
