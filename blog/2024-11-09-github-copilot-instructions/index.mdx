...
:::info
At the time of writing, this is currently under Public preview and the experience mentioned during this article, may change in the future.
:::

:::warning
Custom instructions are currently only supported for Copilot Chat in VS Code and Visual Studio. In our article, I will be using Visual Studio Code, inside of a GitHub Codespace.
:::
...
Once enabled, we need to create our custom instruction file, this file will be named `copilot-instructions.md` and will be placed in the  `./github` directory in the root of our repository _(./github/copilot-instructions.md)_.
...
As we can see, GitHub Copilot references the custom instruction file and provides the output we are looking for.
...
:::info
Using Custom instructions is great for providing context to Copilot, especially adding more detail around what you expect as outputs, and can work really well when you are working with a team, or have a specific way you want your code to be written, however your custom prompts is secondary to the standard system prompt that Copilot uses, so if you provide a custom prompt, it will be used in ADDITION with the standard prompt, and not replaced. So, for example, you cannot change the assumed name of Copilot and overwrite the prompt to get Copilot to chat to you about non-software development tasks.

An example of what the GitHub prompt is as below:
...
We can see some example GitHub Actions and Azure Pipeline workflows that have been outputted, that are could be used as a base to deploy our code.
...
My custom prompt, is overly large and includes a lot of information _(and to be frank I am trying to do alot of different things in this particular repository as its for IaC, where one day it could Terraform and the next Bicep)_, but it is a good example of how you can provide custom instructions to GitHub Copilot to help it understand what you are trying to do, to help keep our code standardised and help improve the quality of the outputs.

:::tip
If you are looking for guidance, on how to develop your own custom prompt. Consider the use of another prompt, to help build it out. An example of a great prompt is one that commonly known as 'One Prompt To Rule Them All', which you could run in ChatGPT or Azure OpenAI Playground to help build out your prompt.
...
Hopefully this article helps you get going, and using custom instructions, and help provide more standardised outputs.