...
keywords:
  ...
  - index naming
  - troubleshooting
description: Chase down why Drasi's Azure AI Search backend crash-loops to a one-character bug, then fix it with a rewrite proxy, ingress-nginx, cert-manager, and a nip.io hostname.
date: 2026-09-30T09:00:00+12:00
categories:
  - Azure
---

I was wiring up Drasi's SyncVectorStore reaction against Azure AI Search when it crash-looped on startup with an error that looked, on its face, like a straightforward Azure AI Search naming restriction. It wasn't. What actually happened was a one-character bug baked into two different places in the reaction's own code, and there was no YAML setting, secret reference, or config flag anywhere that could route around it. Getting it working meant reading the reaction's source, proving the exact constraint with a live REST test rather than trusting the error text, and then building the fix as network infrastructure instead of waiting on an upstream release.
...
This isn't a disposable self-test. It's the sync-point bookkeeping the reaction depends on for its entire lifetime, created fresh on every bootstrap. A hypothetical upstream fix limited to just the self-test's name would still leave AzureAISearch unusable, because this second collection hits the exact same wall. Both prefixes needed the same one-character correction.
...
- [Azure AI Search - index naming rules](https://learn.microsoft.com/rest/api/searchservice/naming-rules)
