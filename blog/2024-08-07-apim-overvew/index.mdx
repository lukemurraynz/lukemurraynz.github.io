...
authors:
  - Luke
slug: azure/api-management-overview
keywords:
  - azure
  - cloudnative
  - integration
...
APIs are at the heart of digital transformation, enabling connected experiences and smooth integration of services and data.

![Azure API Management - Digital Transformation](images/apim_overview_digitaltransformation.gif)

APIs connect devices and platforms. Whether it's a smart home system, wearable technology, automotive applications, or industrial equipment, APIs enable communication and interoperability between these diverse endpoints.
...
APIs also enable businesses to be agile and adaptive. By exposing functionalities through APIs, companies can quickly integrate new services, scale their operations, and meet evolving customer demands. They empower developers to build strong applications, enhance user experiences, and drive digital growth.
...
- **Front Door Control**
This aspect is about managing and controlling how APIs are accessed and used.
...
Secure and Protect: Security is essential in API management. Front Door Control involves implementing robust security measures to protect APIs from threats and unauthorized access. These measures include authentication, authorization, and data encryption.
...
Employees, Partners.
- **Customers**
These stakeholders use the APIs to enhance workflows, integrate systems, and improve customer experiences.
- **Apps on Devices**
...
 
- **User Plane**
Developer Portal: This portal is the primary interface for app developers. It provides resources for developers to discover, learn, try, and onboard with APIs. The portal ensures developers have the support they need to integrate APIs efficiently into their applications.
- **Data Plane**
...
Azure API Management integrates with various Azure services and microservices, providing a cohesive environment for managing and deploying APIs. This integration allows organizations to use the full power of Azure's cloud capabilities, ensuring that their APIs are scalable, secure, and performant.
...
In closing, Azure API Management is a platform that enables organizations to create, manage, and scale APIs efficiently. By following the API life cycle phases and using Azure API Management's features, businesses can deliver smooth digital experiences, drive innovation, and stay ahead.
...
* Full Set of Features: These tiers offer complete features without governance limits, providing full capabilities for complex and large-scale API management needs.

![APIM Pricing Tiers](images/apim_pricing_tiers.png)

Let's look at the key features of each tier.
...
* Availability zones: Use availability zones to increase reliability.
...
This lifecycle ensures that APIs are well-designed, developed, secured, published, scaled, monitored, and analyzed, providing a thorough approach to API management.
...
In addition to the Azure portal, you can also use the Visual Studio Code extension. This extension integrates smoothly with Azure, allowing you to manage and define your APIs directly within your development environment. It's an excellent tool for developers who prefer to work locally.
...
Adopting a design-first approach can significantly enhance efficiency and collaboration. One technique we can use is mocking the API. Let's explore how this approach can benefit our development workflow.

Design-First Approach - Mock the API:

1. Unblock Front-End Teams:

Mocking API responses is a major productivity boost for front-end developers.
...
[WebSocket API](https://learn.microsoft.com/azure/api-management/websocket-api?tabs=portal&WT.mc_id=AZ-MVP-5004796) support in Azure API Management opens up new possibilities for real-time applications. Here is what it offers and how it benefits our API infrastructure.
...
Once the connection is established, APIM smoothly proxies WebSocket messages to the backend services, ensuring smooth and reliable communication.
...
It provides thorough documentation, code samples, and testing tools, making it easier for developers to integrate APIs into their applications.
...
API providers can ensure their APIs are reliable, secure, and up-to-date through the Azure Portal.
...
API Management acts as a mediator, enabling smooth communication between the products and the backend services.
...
API Management ensures that products smoothly interact with cloud-based and on-premises backend systems.
...
Provides a developer-friendly environment with thorough documentation, testing tools, and self-service capabilities.
...
Here are some of the out-of-the-box policies
...
Send one-way request / request: Handles sending HTTP requests.
...
Send request to a service / message to a pub/subtopic: Handles interaction with Dapr services and pub/subtopics.
...
The Azure API Management Visual Studio Code extension provides a smooth experience for managing APIs directly from your development environment.
...
Here are the deployment options for Azure API Management.
...
:::info
Azure API Management has a [Landing Zone Accelerator](https://learn.microsoft.com/en-us/azure/cloud-adoption-framework/scenarios/app-platform/api-management/landing-zone-accelerator?WT.mc_id=AZ-MVP-5004796), a set of pre-built configurations and proven patterns for deploying Azure API Management securely.
...
Securing backend APIs in an interconnected digital environment is essential. One key strategy is strong data plane security — a layered defense that protects sensitive data and applications from unauthorized access. Here is how data plane security works and the roles of its components.
...
APIs _(Application Programming Interfaces)_ enable smooth interaction between software applications. However, managing multiple APIs can be challenging. Bundling APIs with products has emerged as an effective strategy to streamline this process.
...
Azure API Management's multi-region deployment offers a strong solution for improving API reliability and reducing latency. Let's look at the key features and benefits.
...
In a multi-region deployment, the primary region hosts all components, including the gateway, developer portal, management API, and developer portal API. If the primary region becomes unavailable, these components are inaccessible, highlighting the importance of a reliable primary region setup.
...
Here is a look at the anatomy of an Azure API Management instance.
...
In a multi-region deployment, using Availability Zones can enhance the resiliency of the primary region. Even if one zone experiences an outage, the services can operate smoothly from another zone within the same region.
...
* Dapr can work with self-hosted Azure API Management gateways and invoke services running in different pods. This allows for smooth communication between microservices within the Kubernetes cluster. 
