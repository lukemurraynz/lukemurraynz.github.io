...
So lets dig a bit deeper into the details of Azure Extended Zones in Perth, Australia.
...
This command will return the details of the Extended Zones in Perth, including the name, location, and parent _(Home)_ region, including the longitude and latitude.
...
![Register Extended Zone](images/RegisterAzEdgeZonesExtendedZone.gif)

:::warning
You may have to wait a few minutes for the registration to complete,  you can run a `Get-AzEdgeZonesExtendedZone -Name 'Perth' | Select-Object RegistrationState` command to check the status of the registration.
...
Once deployed, you can create resources in the Extended Zone using the Azure portal, Azure CLI, or Azure PowerShell. For our purposes, we will use Azure PowerShell to create a Storage account in Perth Extended Zone.
...
Its worth noting, that if there is an issue in the Australia East region, the Extended Zones in Perth will not be affected, as they are self-sufficient and can continue to operate independently, however you may not be able to create or manage resources in the Extended Zones until the issue is resolved in the Australia East region.
...
In conclusion, Azure Extended Zones in Perth, Australia provide a new option for customers to run their workloads closer to home, with low latency and data residency. The Extended Zones are connected to the Azure backbone network, ensuring fast and reliable connectivity to other Azure regions, however they are not self-sufficient and require a connection to an Azure region _(classed as the parent region)_.
