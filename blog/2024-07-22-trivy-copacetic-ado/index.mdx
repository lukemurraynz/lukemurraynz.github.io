...
:::info
[Copa](https://github.com/project-copacetic/copacetic) gives the ability to patch containers quickly without going upstream for a full rebuild. As the window between [vulnerability disclosure and active exploitation](https://www.bleepingcomputer.com/news/security/hackers-scan-for-vulnerabilities-within-15-minutes-of-disclosure/) continues to narrow, there is a growing operational need to patch critical security vulnerabilities in container images so they can be quickly redeployed into production.
...
:::warning
Copa currently only supports OS package vulnerabilities, and does not support application dependencies or language-specific packages. It is recommended to use Copa in conjunction with Trivy to patch OS package vulnerabilities and use other tools to patch application dependencies and language-specific packages, when validated and tested with developers.
:::
...
* Copa supports patching existing container images _(Devs don't need to build their images using specific tools or modify them in some way just to support container patching)_.
...
graph TD
    ...
    B -->|image| C[Image To Patch]
    B -->|trivyVersion| D[Trivy Version]
    B -->|copaVersion| E[Copacetic Version]
    B -->|buildkitVersion| F[BuildKit Version]
    B --> G{Variables}
    G -->|buildkit.port| H[BuildKit Port]
    G -->|serviceConnectionName| I[Service Connection Name]
    G -->|system.debug| J[System Debug]
    G --> K[Stages]
    K --> L[Stage: Patch]
    L --> M[Job: Copacetic]
    M --> N[Step: Download and Install Trivy and Copa]
    M --> O[Step: Run Buildkit]
    M --> P[Step: Login to ACR]
    M --> Q[Step: Scan image for DevOps Output]
    M --> R[Step: Scan and Patch Image]
    M --> S[Step: Upload Scan Results as a pipeline artifact]
    M --> T[Step: Upload patched image as a pipeline artifact]
    M --> U[Step: Push Patched Image to ACR]
    U --> V[End]
```
...
For demo purposes, I am using an older version of the `[api-firewall](https://hub.docker.com/_/api-firewall)` image, which has known vulnerabilities that need to be patched.

Lets take a look at the image in Azure Container Registry before the patch.

![API FW Before Patch](api-firewall-acr-beforecopapatch.png)

Now, lets deploy the pipeline and see the results.

![API FW Before Patch](api-firewall-ado-beforecopapatch.png)

![Copa ADO pipeline](copa-pipelinerun.gif)

After the pipeline has run, we can see the patched image in Azure Container Registry.
