...
* A Delete of the Network Interface was done by `Luke@geek` - which is my user account.

## Can we determine what made the change?

> Yes, we can determine what made the change. Change Analysis provides the `clientType` field, which tells us what made the change, however as we can see in the previous screenshots this is not always as accurate as we may want it, however its good enough that you can infer what may have issued the changes.

Some of the clientType values you may see are:

* `Azure Portal` - Changes made from the Azure Portal.
* `Unknown` - Change happened on a client that is unrecognized.
