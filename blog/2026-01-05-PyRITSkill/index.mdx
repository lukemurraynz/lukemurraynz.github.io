...
Let us execute this using GitHub Copilot to generate a random system prompt and verify it with PyRIT. Just creating the prompt will trigger the PyRIT skill. Once the Skill is read, it automatically imports the environment variable into the terminal window, which then runs and tests the prompt against various attacks via the Azure OpenAI endpoint.
