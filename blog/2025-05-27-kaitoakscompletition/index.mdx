...
The other day I wrote an article about [Deploy & Test KAITO on AKS with Visual Studio Code](https://luke.geek.nz/azure/aks-kaito-vscode-deployment/) and one of the areas I showcased was testing the completion of the KAITO model within Visual Studio Code, however I didn't touch on how to use the compeition endpoint directly.

{/* truncate */}

In this example, we are gong to connect to the cluster, grab the worksplace, and then run a curl container to test the completion endpoint of the KAITO model:
...
Once connected to the AKS cluster, we can retrieve the service IP of the `workspace-phi-3-mini-4k-instruct` service, which is where the KAITO model is running. This worksplace may change depending on the deployment, so you may need to adjust the service name accordingly.
