---
title: "Kill Your Build-Time Environment Variables: Azure App Configuration with Front Door for SPAs"
...
Because of this issue, I had developed my own Copilot skill, dedicated entirely to diagnosing `ERR_NAME_NOT_RESOLVED` errors from wrong build-time URLs. The fact that this needs its own troubleshooting guide tells you something about how often it goes wrong.
...
The answer is authentication. App Configuration requires credentials to access - either a connection string or a Microsoft Entra ID token. A SPA running in a browser cannot hold either of these securely. You cannot embed a connection string in JavaScript that ships to the client. And you cannot run `DefaultAzureCredential` in a browser - there is no managed identity context.
...
GitHub Actions → npm run build → deploy to Static Web App (once)
                                        ↓
              Same artifact serves staging AND production
