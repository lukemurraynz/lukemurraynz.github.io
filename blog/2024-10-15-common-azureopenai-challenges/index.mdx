...
- Use Azure API Management services as a middle layer between published services and consuming applications and audience |

**Rationale**:

Azure OpenAI itself has minimal capabilities and is reliant on other products to fully capture prompts and inferences for audit and governance purposes.
...
Microsoft’s managed customers are typically large organizations that trust Microsoft and are deemed to be responsible for their actions.
...
Impact – High | - Create an intermediary hop between the client and AOAI service, and allow AOAI access only via Private Endpoint _(Example: AppGW/APIM)_
...
Impact – High | - Each use case to have its own Azure OpenAI instance
