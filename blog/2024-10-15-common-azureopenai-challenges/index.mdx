...
Azure OpenAI Service is API based service, vulnerable to OWASP-10 threats Web Application Firewalls, that typically are used for this, can block significant legitimate AOAI inferencing calls.
...
* Limiting to internal traffic limits the exposure
* Even if an attack is successful from internal network, there is no data stored in service to be compromised
...
All encryption to use [Customer Managed Keys](https://learn.microsoft.com/azure/security/fundamentals/key-management?WT.mc_id=AZ-MVP-5004796). I have personally, haven't seen this requirement yet, but it is a valid requirement, especially for highly sensitive data, and industry _(such as Finance)_ requirements.
...
* AOAI service connection to KeyVault can be over 'Trusted Microsoft Connection' only, and not via the private endpoint of KeyVault.
...
* Use of Keys is typically less secure and unacceptable for long term, although typically increases pace of initial adoption This places use of Service Principal also less secure.
...
|                                                                                                                                                                         |                             | - Avoid using RBAC role with ListKey permissions or use a custom RBAC role                                     |  
...
Azure OpenAI resource must be in a specific region only, this is a common requirement for data sovereignty, and also for compliance reasons.

**Ask**:

Provision Azure OpenAI service in same region where Express Route terminates 

**Challenge**:

[Azure OpenAI and model capacity is highly variable](https://learn.microsoft.com/azure/ai-services/openai/concepts/models?tabs=python-secure&WT.mc_id=AZ-MVP-5004796#model-summary-table-and-region-availability), and typically may not be available in the same region as customers Express Route region.
...
| Why                                                                                          | Risk                         | Solution                                                                                     |  
|----------------------------------------------------------------------------------------------|------------------------------|----------------------------------------------------------------------------------------------|  
| - Network traffic is monitored for all ingress & egress in a predetermined region            | Risk – High Impact – High    | - Enable all regions in policy within a geography acceptable to client                        |  
