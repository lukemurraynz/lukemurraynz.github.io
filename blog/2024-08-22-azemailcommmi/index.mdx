...
In this example, I will access token from [Azure Communication Services](https://azure.microsoft.com/products/communication-services?WT.mc_id=AZ-MVP-5004796). This is done by making a GET request to the identity endpoint of Azure Communication Services, using the oauth identity fromt he system managed identity. This will return a token that we can use to authenticate against the REST API.
...
If there's an error in getting the access token, the script catches the exception and prints the error message and response details to the console.
...
If there's an error in sending the email, the script catches the exception and logs the error message and stack trace to the console.

And further information on the authentication process itself:

1. The script first defines the resource ID for Azure Communication Services and the communication endpoint URL.
2. It then constructs the URI for the identity endpoint. This URI includes the environment variable IDENTITY_ENDPOINT (which is automatically set by Azure when using Managed Identity) and the resource ID.
...
4. If the request is successful, the response will include an access token. This token is then extracted from the response.

This access token can then be used to authenticate requests to Azure Communication Services. The token tells Azure Communication Services that the request is coming from an authenticated source (in this case, the Managed Identity) and should be allowed.
...
You can run this script in an Azure Automation Runbook to send an email using Azure Communication Services, using the System Managed Identity - and no need to maintain or store client secrets!
