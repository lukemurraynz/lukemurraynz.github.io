...
What none of them do well is answer a *relational* question about the live cluster state after admission has already happened: is this Deployment still under-replicated after five minutes, not just right now, and is that specific Pod running an image nobody approved. That gap, standing queries over a graph of live resources, is what I spent time testing Drasi against on a real AKS cluster. The result was more honest than I expected: only two of the six rules I originally set out to build survived contact with a live cluster unchanged, and that gap between what looked right on paper and what actually ran turned out to be the most useful part of the post.
...
The cut rules all trace back to the watch list, not to bad query logic. In their place three new rules earned a spot. Two of them are proven against real violations rather than a toy example; the third I only proved on syntax, and I'll be up front about why when I get to it.
...
That screenshot is the actual live result set, and it caught more than I planned for. It correctly flagged a `docker.io/library/nginx` deployment I'd deliberately mislabelled, but it also surfaced two real-world gotchas any allowlist has to account for on a genuine cluster rather than a toy example:
