...
This is exactly the kind of process that becomes fragile if left to hand-written ad-hoc scripts per repo.
...
This is the fast path from empty folder to deployed Drasi components:
...
> **Cost note:** `azd drasi provision` can create billable resources (especially AKS and Log Analytics). Use a dedicated dev/test subscription or budget guardrails for experimentation. The below are example costs only to give a view of cost, Azure Developer CLI shines with removal and redeployment of entire environments.
...
|---|---|---:|---:|---:|
| AKS nodes ×2 | Standard_D2s_v5 | $5.76 | $40.32 | $172.80 |
| PostgreSQL compute | Standard_B1ms (Burstable) | $0.62 | $4.37 | $18.72 |
| NAT Gateway | Standard | $1.08 | $7.56 | $32.40 |
| Public IP | Standard Static | $0.12 | $0.84 | $3.60 |
| **Total** | | **$7.58** | **$53.09** | **$227.52** |
...
_Key Vault (Standard) and Log Analytics are consumption-based: Key Vault is negligible for dev use; Log Analytics adds ~$3.34/GB (AU East) / $3.51/GB (NZ North) above the 5 GB/day free allowance. VNet and managed identities are free._

If you add an Azure Container Registry (ACR), `australiaeast` and `newzealandnorth` carry identical registry pricing (USD):

| Tier | 1 day | 7 days | 30 days |
|------|------:|-------:|--------:|
| Basic | $0.17 | $1.17 | $5.00 |
| Standard | $0.67 | $4.67 | $20.00 |
| Premium | $1.67 | $11.67 | $50.00 |

_Data storage adds $0.10/GB per month across all tiers. [Current pricing →](https://azure.microsoft.com/pricing/details/container-registry/?WT.mc_id=AZ-MVP-5004796)_
...
These are the scenarios I hit most often when building demos and internal proof-of-concepts.
...
If you want fast feedback in pull requests:
...
- Use `--output json` in automation jobs where you need machine-readable state.
