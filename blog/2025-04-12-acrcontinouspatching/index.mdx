...
```bash
az acr supply-chain workflow show -r azcontainerregistrytest -g azcontainerregtestaue -t continuouspatchv1  -o table
```
...
These three tasks work together to form a complete patching pipeline: the trigger starts on schedule, the scanner identifies vulnerabilities, and the patcher fixes them automatically.
