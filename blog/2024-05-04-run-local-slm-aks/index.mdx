---
title: Run SLM (Small Lanuage Model) on AKS (Azure Kubernetes Service)
...
Today, we are going to look at deploying a Small Lanuage Model *(SLM)* directly into your AKS *(Azure Kubernetes Service)* cluster, running on GPU enabled nodes, using [Kaito *(Kubernetes AI Toolchain Operator)*](https://github.com/Azure/kaito).
...
* Workspace controller: It reconciles the workspace custom resource, creates machine custom resources to trigger node auto provisioning, and creates the inference workload (deployment or statefulset) based on the model preset configurations.
...
Today we are going to use the [aks store demo - Pets](https://github.com/Azure-Samples/aks-store-demo) sample microservices app, commonly used for AKS demos, tutorials and experiments and deploy it to our AKS cluster, and update it to use a locally hosted mistral model to generate the local product descriptions, then calling the externally hosted Azure OpenAI endpoint.
...
* [Request a quota increase](https://learn.microsoft.com/azure/quotas/regional-quota-requests?WT.mc_id=AZ-MVP-5004796) for a GPU compute workload *(ie Standard NCSv3)* in your region. I am based in New Zealand, so will be deploying to Australia East.

### Deploy AKS cluster

1. Lets deploy our Cluster and application, by using a pre-created Azure Developer CLI infrastructure as code deployment, by connecting to the Codespace *(which has the depedencies, of Azure Developer CLI, Kubectl, Helm, Azure Bicep, Terraform Azure CLI, already pre-installed)* at [aks store demo - Pets](https://github.com/Azure-Samples/aks-store-demo).
![Open Codespace](Kaito-AKS_Codespace_Open.gif)
2. Now we need to prep for the Azure Developer CLI deployment, which will use Terarform (random pet name module as well for unique name) to deploy our AKS cluster in our specified region. So lets login to Azure.
...
4. Enter in an environment name *(ie Kaito)*, this is used by the Azure Developer CLI to store the environment settings, for the deployment, and won't reflect any of your deployed resources.
5. Now we need to set a few environment variables, for our deployment, which will be used by the Terraform deployment to deploy the Azure Open AI instance that Pets will use *(and we will replace with Mistral later on, however, I recommend including it in the deployment, to confirm that the OpenAI components are confirmed working first)*.
...
To confirm variables you can run:
...
6. Finally its time to deploy, run the following command and select your Subscription, you want to deploy the AKS cluster into and the region *(remember this region is the region that you have requested the GPU quota increase for)*.
...
7. Once the deployment is complete, you will see the output of the deployment (AKS and helm deployments for the PET microservices), including the AKS cluster name, and the Azure OpenAI endpoint, the OIDC issuer URL.
...
1, Open your favourite browser and navigate to the [Azure Portal](https://portal.azure.com/#home)
2. Search for: Kubernetes services
3. Find your AKS cluster, and click on the resource
4. Click on Services and inpresses
5. Find the store-admin service, and click on the public endpoint, to open the store admin page.
6. You can test the current *(external to the AKS cluster)* Azure OpenAI endpoint, by clicking on Products, Add Product, type in a Keyword, click on the "Ask AI Assistant" button, and see the generated text.

![AKS - Test Pet store admin OpenAI endpoint](Kaito-AKS_PetStoreAdminAzOpenAITest.gif)

### Implement Kaito and Mistral LLM

Now that we have the AKS cluster deployed, and the Pets store app running, we can now implement Kaito and run the Mistral 7b-instruct Large lanuage model.

> You can continue to set this up in the Codespace, I am going to switch to an [Azure CloudShell](https://learn.microsoft.com/azure/cloud-shell/overview?WT.mc_id=AZ-MVP-5004796), to complete the rest of the implementation, mainly to come at the implementation, from a point of view of installing Kaito in an existing cluster, not related to the Pet store example *(ie make sure that there are no variables, etc that we might be reliant on, without knowing why)*. 

1. To do, we need to make sure that we have the preview version of the AKS cli extension installed. You can check this by running the following command:
...
```
2. Re-login to Azure, if needed  *(you don't need to login to the Azure Developer CLI, was only required for previous steps)*. Next we need to register the AIToolchainProvider resource provider, by running the following command:

```bash
az feature register --namespace "Microsoft.ContainerService" --name "AIToolchainOperatorPreview"
```

This will allow the Kaito (AI Toolchain Operator) APIs on your Azure subscription, to allow it to be deployed into your AKS cluster. It could take a few minutes to register.

![Register AI Tool Chain Operator](Kaito-AKS_RegisterAIToolChainOperator.gif)

3. Once the registration is complete, we can now prepare to deploy the workload identity, workspace and the GPU enabled nodepool for Kaito, as part of this, we need to define the environment variables (if you are still using the Codespace, these should be carried across from the previous deployment, if not then we can set them)*. Adding the variables, helps us without having to enter in the values each time we run a command.
...
Kubectl is the Kubernetes command-line tool that allows you to run commands against Kubernetes clusters. You can use kubectl to deploy applications, inspect and manage luster resources, and view logs. It's a crucial component for interacting with Kubernetes and is used in various operations, from creating, updating, and deleting Kubernetes resources to debugging running applications.
:::

8. Lets assigned the managed identity, Contributor rights over the Resource Group, that contains the Cluster, by running the following command:
...
I had issues, where the gpu-provisioner pod was not running and kept constantly restarting, and the logs showed that the identity was not found, so I had to re-run the command to assign the correct managed identity, and using the echo command, to reveal the managed identity highlighted the wrong one was selected.

:::

![kaito-gpu-provisioner | Azure Portal overview](kaito-gpu-provisoner-deployment-azportal.png)

9. Lets now deploy the Mistral 7b instruct model, to our AKS cluster.

:::warning

As soon as the models are grabbed from the remote registry, it will spin the GPU nodes up and you will start paying for their use, so just be aware.

:::

We are going to use one of the supported models, to create our interference endpoint in AKS, and will use the Mistral 7b instruct model, as per their examples in the K[aito GitHub repository](https://github.com/Azure/kaito/tree/main/examples/inference).
...
Once the workspaces and pods/nodes have been provisioned, lets test the deployment, by running the following command:
...
### Update Pet store app

Almost there! Now we know that our local chat endpoint is working with the Mistral 7b instruct model, we can now update the Pet store app, to use the local Mistral 7b model, and remove the Azure OpenAI, to generate the product descriptions.

To do that, we will continue in our Terminal, but like the previous steps, can be done in the Codespace. The Pet Store application, already has some great examples on doing this, so we will create a new manifest and repoint the application to use the local Mistral workspace, instead of the public Azure Open AI endpoint that was originally created:
...
Now lets test it! We will remove the Azure OpenAI instance first, then test it!
