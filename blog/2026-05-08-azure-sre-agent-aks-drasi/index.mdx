...
The result is an [Azure SRE Agent with support for Drasi on AKS](https://github.com/lukemurraynz/drasi-aks-sre-agent/), that can be deployed using an `azd up` with an AVM style Bicep module and PowerShell.
...
That is where [SRE Agent](https://learn.microsoft.com/azure/sre-agent/overview?tabs=task&WT.mc_id=AZ-MVP-5004796) becomes interesting. I had a lot of fun setting this up, and the mind boggles on what this can do!
...
The [lukemurraynz/drasi-aks-sre-agent](https://github.com/lukemurraynz/drasi-aks-sre-agent/) repository can help you deploy the resources for SRE Agent with Bicep and wires the agent configuration through a post-provision script.
...
I did not want the Drasi runtime agent debugging a cluster-wide scheduling issue. I also do not want the AKS agent deleting Drasi resources because a query or source isn't quite working.
...
:::warning
Autonomy should be route-specific. Do not make the whole agent autonomous because one remediation is safe enough for your environment.
:::

[Azure SRE Agent - Stopped Cluster](images/AzureSREAgent_DrasiAKS_ShutdownStartupClusterTest.gif)

The Alert then changed to Acknowledged, and the Agent will output a Kepner-Tregoe problem management table _(ie IS vs IS NOT)_.

We can even have a look at the Trace of the process, to see the steps it took, this can help us improve the Agents and its Skill calling:

[Azure SRE Agent - Stopped Cluster Trace](images/AzureSREAgent_DrasiAKS_ShutdownStartupClusterTestTrace.gif)

## Session Insights

Every incident creates an investigation session you can open in the portal. I found these worth going back and reading properly after each test run.
...
That helps surface slow tool calls or failed skill invocations the session view does not always make obvious.
...
The Trace view shows the order of skill calls and subagent handoffs. If a route touched three agents before finding the right one, the triage logic in `drasi-incident-triage` needs adjusting.
...
Keep it off by default. Turn it on for validation windows only.

:::danger
Always-firing synthetic alerts left running permanently will trigger autonomous or review-mode agent runs continuously, burning through token and tool usage. Deploy them, validate, then delete or disable.
:::
...
3. Delete and recreate only the affected Continuous Query if bootstrap failed.
...
### 1. Route by failure phase before product
...
- Metrics blindness usually means metrics API or monitoring pipeline.
...
Deploy them behind a flag, run the validation, capture the evidence, delete them.

### 4. "Connected" is not the same as "usable"

MCP connectors can be connected and healthy while their tools are not active for the agent.
...
From a [Well-Architected Reliability](https://learn.microsoft.com/azure/well-architected/reliability/?WT.mc_id=AZ-MVP-5004796) perspective, this is about reducing detection and diagnosis time without blindly increasing automation risk.
...
From a Cost Optimization perspective, it also matters because noisy autonomous agents can burn through token and tool usage quickly. Route narrowly, scope tools, and keep high-impact flows in Review until you have real evidence.
...
That is exactly the kind of ambiguity SRE Agent can help with, as long as we give it the right guardrails.
