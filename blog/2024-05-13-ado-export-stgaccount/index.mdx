...
Today, we will use an Azure DevOps pipeline to run a PowerShell script. The script will connect to the Azure DevOps API and grab the zip file of each Repository in all Projects into an artifact, which then gets exported and copied to an Azure Storage Account.
...
if (Test-Path -Path "$env:SYSTEM_DEFAULTWORKINGDIRECTORY/repositories") {
    ...
    Write-Host "Repositories directory does not exist, cannot upload as an artifact"
}
