---
title: Integrating Microsoft Learn Docs MCP with Semantic Kernel
metaDescription: Learn how to integrate Microsoft Learn Docs with the Semantic Kernel for enhanced documentation and AI capabilities.
tags:
  - Azure
categories:
  - Azure
authors:
  - Luke
slug: azure/integrating-microsoft-learn-docs-mcp-semantic-kernel/
keywords:
  - Azure
  - Semantic Kernel
  ...
  - AI Documentation
  - Microsoft Graph
description: Learn how to integrate Microsoft Learn Docs with the Semantic Kernel for enhanced documentation and AI capabilities.
date: 2025-09-27
---

I was working on a content discovery app recently and ran into an annoying problem. The AI component needed access to current Microsoft documentation, but I wasn't keen on scraping their site (and dealing with all that maintenance overhead).

{/* truncate */}

Then I discovered Microsoft's [Model Context Protocol (MCP)](https://learn.microsoft.com/training/support/mcp?WT.mc_id=AZ-MVP-5004796) server for Learn docs. MCP was new to me - it's essentially a standardized way to connect external data sources to AI applications. Microsoft maintains their own MCP server that stays current with their documentation, which solved my problem perfectly.
...
![Learn MCP](images/MCP.jpg  "Microsoft Learn Docs MCP")
...
2. SK: "This needs Microsoft docs, let me call the MCP plugin"
3. MCP plugin hits Microsoft's server with the search
...
The plugin class itself is pretty simple - just needs an HttpClient and some basic setup:
```csharp
...
This is where it gets interesting. The MCP protocol is just JSON-RPC 2.0 over HTTP, so I built a simple wrapper that Semantic Kernel can call:
    ...
    [Description("Language locale for localized content (e.g., en-us, es-es, fr-fr)")] string locale = "en-us")
{
    try
    {
        ...
        {
            _logger.LogWarning("MCP server returned error {StatusCode}: {Content}", response.StatusCode, responseContent);
            // Note: Return JSON instead of throwing - SK handles this better
            var fallback = new { success = false, error = $"MCP server error: {response.StatusCode}", query = query };
...
There are a couple ways to call this thing:
...
The MCP server sends back JSON that you can parse if you need to do something specific with the results:
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            return response.IsSuccessStatusCode 
                ...
                : HealthCheckResult.Unhealthy("MCP server returned non-success status");
        }
        catch (Exception ex)
        {
            return HealthCheckResult.Unhealthy($"MCP server check failed: {ex.Message}");
        }
    }
}
```
...
2. Calls the MCP plugin to search for current scaling documentation
...
**Rate limits** - Don't spam the endpoint. Microsoft is reasonable about limits, but implement retry logic for production use.

**Timeouts** - Some searches can be slow. I use a 30-second timeout to avoid hanging the application.

**Error handling** - Errors might come back in the JSON-RPC response rather than HTTP status codes, so check both.

**Localization** - The `locale` parameter works well - `fr-fr` will return French documentation when available.


## Wrapping up

This turned out simpler than expected. No scraping, no stale docs, no index maintenance - just direct access to Microsoft's current documentation through their MCP server.
...
The MCP ecosystem is growing, and I expect more providers will adopt it. The protocol's simplicity makes it easy to support multiple MCP servers in a single application.

For applications requiring Microsoft documentation, this approach is more reliable than scraping or maintaining local caches. The implementation above should provide a solid starting point.
