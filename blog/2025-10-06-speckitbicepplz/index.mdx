...
::: note
Nothing has been actually created yet — no Bicep, no infrastructure — which is the point of the Spec-Driven Development approach. The focus is on the "what" and "why" before the "how", and it helps flesh out the actual requirements.
:::
...
:::note
No Bicep has been created yet; we are still in the planning stage. This ensures a clear understanding of what we want to build before implementing the technical solution. The next step is to generate the tasks, which will break down the plan into concrete tasks for implementation.
:::

## Breaking down into tasks

The next stage is to create the tasks, which will break down the plan into concrete tasks.
...
Now, proceed to the implementation stage, where we will generate the Bicep code.
...
Now, take a look at the Bicep it created.
...
If I were to repeat this process from scratch _(outside of this blog article)_, here are some of the things I would do differently:
