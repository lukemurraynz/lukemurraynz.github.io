...
When developing a python application that interacts with Azure OpenAI, you may want to authenticate with an API key for testing, but in Production you should make use of Managed Identities within Microsoft Azure.

In this article, we will look at how to authenticate with [Azure OpenAI](https://learn.microsoft.com/azure/ai-services/openai/overview?WT.mc_id=AZ-MVP-5004796) using an API key in Python for local development, then using an environment variable switch to authenticating using Entra ID managed Identities.

<!--truncate-->

The true hero for Azure authentication is the [DefaultAzureCredential](https://learn.microsoft.com/python/api/azure-identity/azure.identity.defaultazurecredential?view=azure-python&WT.mc_id=AZ-MVP-5004796) class from the Azure Identity library. This class is capable of authenticating with Azure services using a variety of methods, including Managed Identity, Shared Token Cache, and environment variables.
...
2. **Check for Missing Variables**: It defines a list of required environment variables and checks if any of them are missing. If any required variables are not found, it raises an error indicating which variables are missing.

3. **Authentication Setup**: The code checks if the `AUTHENTICATION` environment variable is set to `AZURE`. If it is, it uses Azure's DefaultAzureCredential to get a bearer token for authentication. Otherwise, it uses an API key for authentication.
