...
* Data Management—Data management patterns address challenges related to data consistency, synchronization, and management across different locations, which is crucial for cloud applications. Sustainable data practices ensure data is stored and processed efficiently, minimizing energy use and reducing environmental impact.

#### Acroymns

When talking about Design Patterns, we will be using a few acronyms, and it is important to understand what they mean to fully understand the context.
...
:::warning
As usual, the choice of pattern depends on the specific requirements of your application. It's essential to understand the characteristics and trade-offs of each pattern and business requirements, to make an informed decision. In some cases, multiple patterns may be required, or even part of one, to solve a specific problem.
:::

### Pattern Deep Dives

The cloud has changed the way we design our applications. Let's examine some of the design patterns that we can use in the cloud. These patterns help us solve common problems in cloud architecture and make our applications more reliable, scalable, and secure.

In the following sections, we will examine some common cloud design patterns, their use cases, and considerations in depth.
...
```mermaid
graph TB
  Start(Start) --> NoNativeCacheOps{Does in-built cache not provide native read-through or write-back operations?}
...
If the data is not found in Azure Cache for Redis (cache miss), the application will need to retrieve it from the appropriate Azure database service.
...
```mermaid
graph TB
  Start(Start) --> IsScaleBeyondSingleNode{Is data store likely to scale beyond resources of a single node?}
...
Data stores have the ability to handle the upload and download of data directly without requiring that the application perform any processing to move this data. But, this typically requires the client to have access to the security credentials for the store. This can be a useful technique to minimize data transfer costs and the requirement to scale out the application, and to maximize performance. It means, though, that the application is no longer able to manage the security of the data. After the client has a connection to the data store for direct access, the application can't act as the gatekeeper. It's no longer in control of the process and can't prevent subsequent uploads or downloads from the data store.
...
graph TB
  ...
  NeedToOffload -- Yes --> ServerNotOptimized{Is application server not optimized for serving static content?}
  NeedToOffload -- No --> DoNotUsePattern[Do not use this pattern]
  ServerNotOptimized -- Yes --> HeavyLoad{Is application server under heavy load?}
...
* Supporting Cloud or Cluster Connectivity Requirements - When you need to support cloud or cluster connectivity requirements in a legacy application or an application that is difficult to modifyBy considering these scenarios, you can effectively use the Ambassador pattern to enhance the networking capabilities, security, and maintainability of your applications.
...
```mermaid
graph TB
  Start(Start) --> GradualMigration{Gradual migration of a legacy system?}
  GradualMigration -- Yes --> UsePattern[Use Strangler Fig pattern]
  GradualMigration -- No --> ComplexSystems{Dealing with complex systems?}
  ComplexSystems -- Yes --> UsePattern
  ComplexSystems -- No --> MaintainFunctionality{Maintain functionality during migration?}
  MaintainFunctionality -- Yes --> UsePattern
  MaintainFunctionality -- No --> AvoidBigBangRewrites{Avoid "big bang" rewrites?}
  AvoidBigBangRewrites -- Yes --> UsePattern
  AvoidBigBangRewrites -- No --> ModernizeLegacySystems{Modernize legacy systems?}
  ModernizeLegacySystems -- Yes --> UsePattern
  ModernizeLegacySystems -- No --> UseModernOrchestrationTools{Use modern orchestration tools?}
  UseModernOrchestrationTools -- Yes --> UsePattern
  UseModernOrchestrationTools -- No --> DoNotUsePattern[Do not use Strangler Fig pattern]
```
...
```mermaid
graph TB
    Start(Start) --> UndoingOperationsInEventualConsistencyModels{Undoing Operations in Eventual Consistency Models?}
    UndoingOperationsInEventualConsistencyModels -- Yes --> UsePattern[Use Compensating Transaction pattern]
    UndoingOperationsInEventualConsistencyModels -- No --> HandlingFailuresInLongRunningTransactions{Handling Failures in Long-Running Transactions?}
    HandlingFailuresInLongRunningTransactions -- Yes --> UsePattern
    HandlingFailuresInLongRunningTransactions -- No --> EnsuringReliabilityAndResilience{Ensuring Reliability and Resilience?}
    EnsuringReliabilityAndResilience -- Yes --> UsePattern
    EnsuringReliabilityAndResilience -- No --> ComplexBusinessProcessesAndWorkflows{Complex Business Processes and Workflows?}
    ComplexBusinessProcessesAndWorkflows -- Yes --> UsePattern
    ComplexBusinessProcessesAndWorkflows -- No --> AvoidingComplexityOfDistributedTransactions{Avoiding the Complexity of Distributed Transactions?}
    AvoidingComplexityOfDistributedTransactions -- Yes --> UsePattern
    AvoidingComplexityOfDistributedTransactions -- No --> HandlingStateChangesInSOA{Handling State Changes in Service-Oriented Architectures (SOA)?}
    HandlingStateChangesInSOA -- Yes --> UsePattern
    HandlingStateChangesInSOA -- No --> ManualInterventionAndRecovery{Manual Intervention and Recovery?}
    ManualInterventionAndRecovery -- Yes --> UsePattern
    ManualInterventionAndRecovery -- No --> DoNotUsePattern[Do not use Compensating Transaction pattern]
```
