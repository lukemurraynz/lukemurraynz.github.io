...
A response plan with `agentMode: review` also did not prevent execution in my test. Tested twice on a supervised agent, every plan set to review, and it scaled a Deployment without approval. This differs from the current product guidance, which describes Review mode as requiring approval for write operations. Treat that discrepancy as a release-specific behavior to verify in your tenant, not as a supported guarantee either way. RBAC and hooks answer what the agent can do. Governance must answer who decided that it may.
