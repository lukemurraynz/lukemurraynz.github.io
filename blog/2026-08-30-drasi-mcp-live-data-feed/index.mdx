...
The dotted lines are the part that matters: subscribe once, and everything after that is the server pushing to the client, not the client asking again.
...
Worth calling out separately: the `template` value is itself a JSON string nested inside the outer JSON string, so its own quotes need escaping. Get one level wrong and the error message won't tell you which layer broke it, so it pays to build these up one query at a time rather than writing the whole file blind.
...
## The five-minute win, actually tested
...
I then fixed the deployment underneath, outside of MCP entirely, just a normal `kubectl` fix to the thing that was actually broken. A `notifications/resources/updated` message arrived at the client unprompted, no request preceded it, and re-reading the resource confirmed the payload had gone from the under-replicated row to empty. Subscribe once, get pushed a live update, never poll again. That's the pitch, and it held up with a real client instead of the resource model on paper.
...
## The pinned MCP spec and what the client actually negotiated
...
I wanted to know how far this pattern reaches into the tools people actually build agents with, so I checked both the Microsoft Agent Framework SDK and the current Copilot Studio MCP integration rather than guessing. Neither supports it. Worth knowing why before you assume either one will.
