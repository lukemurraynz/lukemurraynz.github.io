...
Recently deleted Azure DevOps code repositories go into a soft-delete state, when deleted. The Repositories are retained for 30 days before they are permanently deleted. However, for Disaster Recovery scenarios, or those scenarios where you may not realise something has been deleted, until after Microsoft retained backup. You may want to export the repositories at a single point in time, to an Azure storage account.

Ironically, the Azure DevOps service itself is highly-reduntant built on-top of core Azure Platform infrastrcuture components, such as Avaliability Zones and relies on Azure Cloud native Storage and Azure SQL services, which are highly-reduntant and backed up services, however mistakes can happen, and there may be organisational requirements to retain backups of the repositories. 

Today we will be looking at exporting your repositories to an Azure Storage Account, using a DevOps pipeline. This pipeline, will capture the repositories at their point in time, and run nightly. 
...
graph TD
  ...
  C --> D[Allow Public Access to Azure DevOps Export Storage Account for upload]
  D --> E[Check Storage Network Access]
  E --> F[Download Build Artifacts]
  C -->|PowerShell Script| G[Export Azure DevOps Repositories]
  G --> H[Upload Repositories to Azure Storage Account]
  ```
...
In order to store the exported repositories, we will need to create an Azure Storage Account. We can point towards an already existing Storage Account, and Container, but for the purposes of this article, we will deploy the Storage account fresh using Azure Bicep to our already existing Resource Group.

:::tip
I will using the [Deployment Pane](https://luke.geek.nz/azure/Azure-Bicep-Deploy-Pane/) to deploy the Storage Account, straight from Visual Studio Code, running in a [Codespace](https://luke.geek.nz/azure/Getting-Started-with-GitHub-Codespaces/).
:::
...
Make note of the Storage Account name and Container Name, we will need them for Azure DevOps pipeline variables.

## Azure DevOps Pipeline

Now, its time to setup your Project and repository, if you don't know how to do that, you can follow the offical Microsoft Learn documentation here: [Create a new Git repo in your project](https://learn.microsoft.com/en-us/azure/devops/repos/git/create-new-repo?view=azure-devops&WT.mc_id=AZ-MVP-5004796). 
...
:::tip
The PowerShell script, can also be ran locally. It will automatically, collect the variables from the Pipeline variables, if it detects an Azure DevOps agent, else those variables can be added manually to the script, to be able to run it locally.
...
Once, commited. Its time to setup our pipeline.
...
The we need to add to our pipeline are:
...
So lets import our pipeline, and configure our variables!

1. To import, the Pipeline, navigate to your Azure DevOps Project, and click on Pipelines, then click on the "New Pipeline" button.
2. Click Azure Repos Git, and select your repository.
3. Select Existing Azure Pipelines YAML file
4. Select your Pipeline and click Continue
5. Now we can add our Variables in, by clicking Variables, and then Add. If you have already imported the Pipeline, you can navigate to the Pipeline, and click Edit, and then Variables.
...
Now, that you have configured your Storage Account and Pipeline, its time to run the pipeline manually, to make sure there are no problems, before your scheduled time runs.
...
:::info
If a Repository is only used for Azure Boards, and not for code, it will not be exported, as the API, will not return it *ie you will usee a (404) Not Found*.
:::
