...
- A Standard Event Hub Namespace with a event hub instance with 1 partition and 1 consumer group
...
So i've already precreated these resources, but is some more information of the resource configuration, that you can follow along with.
...
The Function App connects to the Event Hub namespace the System Managed Identity of the Function App, which is granted the `Azure Event Hubs Data Receiver` role on the Event Hub namespace.

![Function App Configuration](images/Function_SMI_EventHubDataReceiver.jpg)

In order to use the Event Hub trigger in the Azure Function with Managed Identity authentication, you need to add the following settings to the Function App settings :
...
The namespace, Consumer Group and identity type will need to be updated to the function.json file.
...
:::info
You can find the Function App code in the [GitHub repository - lukemurraynz/SFTPEventHubFunction](https://github.com/lukemurraynz/SFTPEventHubFunction) for this blog post.
:::

So lets take a look at it in action!
...
Hopefully thats given you the base to work from, you could potentially add logic around if file from x user, do this, or notify user of receipt based on a map lookup ecosystem
