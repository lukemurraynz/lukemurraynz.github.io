...
In this post, I will walk through a checklist I applied when hardening a .NET project that ran on [Azure Container Apps](https://learn.microsoft.com/azure/container-apps/?WT.mc_id=AZ-MVP-5004796).
...
Multi-stage Docker builds keep build tools (SDK, compilers, npm dev dependencies) out of the runtime image. This reduces attack surface and image size.
...
:::tip
Use `--only=production` (or `--omit=dev` in npm 9+) in runtime stages so TypeScript, ESLint, Vite, and other dev tooling is not shipped to production.
:::
...
Health endpoints should bypass authentication middleware. If readiness requires a JWT, the platform cannot determine service health correctly.
