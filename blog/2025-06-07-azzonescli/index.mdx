...
## Designing for Reliability with Availability Zones
...
I can see I have an Expressroute circuit with the status of Unknown, This means that the zones CLI extension isn't able to verify this resource type. So you would need to check manually - make sure your [virtual network gateways are zone-redundant](https://learn.microsoft.com/en-us/azure/vpn-gateway/about-zone-redundant-vnet-gateways?WT.mc_id=AZ-MVP-5004796). Virtual Networks are zone-redundant by default.

## Scoping Your Validation

When we ran `az zones validate` it ran across all resources across subscriptions in the tenant we have access to. If we want to limit the scope of the scan, we can.
...
_(The tag value pair is case sensitive.)_
