...
There are different types of primitives that an MCP server can expose, that extend the ability of your AI applications and clients to create, read:
...
So in our demo, our client is going to be [Visual Studio Code](https://code.visualstudio.com/), connecting to a MCP server over https to utilize the document search tool, that allows us to get the same content, that services such as Copilot for Azure, has to the Microsoft Learn semantic search index, allowing us to ground our responses in the current documentation, that will know things like Azure Active Directory is now Entra ID, or the services that are generally available within a specific region.
...
Its worth noting as we go into the demo, that MCP servers, expose the tools and resources, but do not run any reasoning logic themselves, the host application and LLM decides when and how to use the tools that are exposed, you can control this behaviour in the desctipion of your MCP servers, and the crafting of your system and user prompt, you will see some of this in our demo shortly.

{/* truncate */}

![Model Context Protocol _(MCP)_](images/mcp.jpg)

So - lets get started, first thing we are going to do is add the [Docs MCP server](https://github.com/MicrosoftDocs/mcp) itself, then we will do some testing and then follow it up with a custom Github Copilot Chat Mode.

First thing, let us start with a Visual Studio Code instance, we will click on GitHub Copilot, select Agent Mode, click on the Configure Tools, and scroll down the bottom for Add MCP server.
Add HTTP server, and add `https://learn.microsoft.com/api/mcp` and specify a name \_(for example: `microsoft.docs.mcp`).

![Configure Learn MCP](images/ConfigureLearnMCPVSCode.gif)

You willl end up with an mcp.json file that looks something like this:
...
- Using Microsoft docs, is Azure Kubernetes Service avaliable in New Zealand North?
...
- Use the `websearch` tool on [Product Availability by Region](https://azure.microsoft.com/en-us/explore/global-infrastructure/products-by-region/table?WT.mc_id=AZ-MVP-5004796) for guidance around Azure product and service avaliability IF the `microsoft.docs.mcp` tool is unsure.
