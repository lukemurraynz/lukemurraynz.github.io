...
When working with Microsoft Azure, you may want an environment for learning, testing or Hackathons. This post will cover some technical implementation considerations for creating a sandbox environment in Azure, that could be used for your own Hackathons using Terraform, Entra ID Access packages and GitHub Actions.
...
> "As a hackthon organizer, I want to build an Azure environment that supports various AI-driven solutions aimed at automating and enhancing operational processes, including customer feedback analysis, testing plan creation, and standard procedure refinement."

![Hackathon - Requirements](images/hackathon_requirements.PNG)

We have a few Technical Requirements:

* Needs to support multiple _(external)_ users and teams from different organisations using Work and Personal accounts, who work as a team.
...
For the purposes of this article, there are some assumptions, I will be making such as:
...
We will look at approaching this, as a [Lightly Managed Sandbox](https://luke.geek.nz/azure/microsoft-azure-sandbox-design-considerations/).

![Hackathon - Sandbox Types](images/hackathon_sandboxtypes.PNG)

And give External Identites, access to an Azure subscription, through Access Packages and a Group with Contributor role assigned to the individual Resource Groups.

![Hackathon - Identity & Access](images/hackathon_identityaccess.PNG)

To do this, we will leverage Access packages, to invite the external users into the Hackathon tenancy, and assign them to a group that has the Contributor role assigned to the Resource Group.

![Hackthon - Access packages](images/hackathon_identityaccesspackages.PNG)

By creating a Policy, allowing External organisations, to request. This will allow the Hackathon Organiser to invite external users, to the Azure subscription, and access internal _(Hackathon)_ resources.
...
So lets take a look at some of the main Terraform code snippets that could be used to create the Azure resources for the Hackathon.

:::info
The identity, being used to create these Terraform resources will need:

* Owner to the Azure Subscription, to create the Resource Groups and role assignments.
...
```
What this does is read a CSV file, that contains the details of the Azure Resource Groups that need to be created for the Hackathon, here is a base CSV file that I have used.
...
Using this, and the [AzureRM](https://registry.terraform.io/providers/hashicorp/azurerm/latest) Terraform provider. We can create the Resource Groups, and assign the Access Packages to the Resource Groups.
...
So now, we have created the Resource Groups, Entra ID groups and assigned Owner permissions to the Resource Groups, for each Team in the CSV file, its time to deploy a base resource - such as Azure OpenAI, which those hackathon teams can use, to get going! [Azure OpenAI models are deployed to various regions](https://learn.microsoft.com/azure/ai-services/openai/concepts/models?WT.mc_id=AZ-MVP-5004796), and have token limits, constrained to the region, so we need to consider how heavily these endpoints will be hit - which is why you may have noticed in the CSV, that the location of each Resource Group is different, so we want to use that to make sure that our Azure OpenAI resources are deployed in the same region as the Resource Group, to help balance.
...
:::warning
The [request policies](https://learn.microsoft.com/entra/id-governance/entitlement-management-access-package-create?WT.mc_id=AZ-MVP-5004796#create-request-policies) themselves, I had issues deploying via Terraform, I suspect due to missing Graph permissions 'EntitlementManagement.ReadWrite.All' on the identity, but had some issues adding this, so did them manually, however have successfully deployed the policies in another environment sucessfully using the: [azuread_access_package_assignment_policy](https://registry.terraform.io/providers/hashicorp/azuread/latest/docs/resources/access_package_assignment_policy) terraform resource. 

Make sure you add:
requests_accepted = true
To the requestors block, to enable the policy, or the policy will be deployed as Disabled.
:::
...
Now we have our base Terraform, its time to deploy this, and we can use GitHub Actions to do this, by creating a GitHub Actions workflow file, such as below:
...
This GitHub Actions workflow, will connect to Azure, using OIDC from a User Managed Identity, and deploy the Terraform plan, and apply the changes, to the Azure subscription.

The Terraform backend settings look like this:
...
You can find the full code for this, in the [lukemurraynz/HackathonRGAzureEnvironment](https://github.com/lukemurraynz/HackathonRGAzureEnvironment). Feel free to open a PR _(Pull Request)_, if you have any suggestions or improvements.
