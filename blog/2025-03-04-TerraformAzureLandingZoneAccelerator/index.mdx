...
Once we have those pre-requisites, we can then proceed with the bootstrapping process. 
...
3. Lets create the local configuration files for the bootstrap and Platform Landing Zone deployment:
...
It should look somethng like below:
...
subscription_id_management: "6bca53bc-98d9-4cd3-92e7-0364c7bffac4"
subscription_id_identity: "6bca53bc-98d9-4cd3-92e7-0364c7bffac4"
subscription_id_connectivity: "6bca53bc-98d9-4cd3-92e7-0364c7bffac4"

# Bootstrap Inputs
azure_devops_personal_access_token: "914tIKvivkfIgTRUcX3l34HMa6CJWJWKqu3AKkznmi74vKU4sDOCJQQJ99BCACAAAAAhGPgtAAASAZDO25KS"
# azure_devops_agents_personal_access_token: "<token-2>"
azure_devops_organization_name: "luke0153"
use_separate_repository_for_templates: true
bootstrap_subscription_id: "6bca53bc-98d9-4cd3-92e7-0364c7bffac4"
service_name: "plz"
...
apply_approvers: ["luke@luke.geek.nz"]
...
Now that we have configured the variables required for our bootstrap, lets define the Terraform variables for the Platform Landing Zone deployment, this will be used to define the Azure resources we will deploy, such as the Hub and Spoke, Virtual WAN, Bastion, etc, and this is the platform-landing-zone.tfvars file. I recommend copying one from one of the [already pre-existing Scenarios](https://azure.github.io/Azure-Landing-Zones/accelerator/startermodules/terraform-platform-landing-zone/scenarios/) and modifying to suit you needs. For my purposes, I will be going with a Single region hub and spoke VNET with [Azure Firewall](https://azure.github.io/Azure-Landing-Zones/accelerator/startermodules/terraform-platform-landing-zone/scenarios/single-region-hub-and-spoke-vnet-with-azure-firewall/).
...
```
:::info
The `platform-landing-zone.tfvars` file is a crucial configuration file for deploying an Azure Landing Zone using the Azure Landing Zones Accelerator with Terraform. This file defines the variables that customize your Azure foundation in accordance with your requirements and this file will be your main configuration file for the deployment of the Platform Landing Zone.
...
And don't worry if you don't get ths right the first time, you can always adjust and redeploy, this is the beauty of Infrastructure as Code. I work in an ilterative approach, so its always good to get something working, then adjust and improve, rather than trying to get it perfect the first time, and the Terraform plan and validate pipelines help with this approach.

6. We are close, the last thing we will add is a custom library folder. This isn't required for base Platform Landing Zones, however, I recommend adding it in, for additinal flexibility, especially as you start to deploy more complex environments and archetypes within your organisation. You can refer to the [Azure Landing Zones Library](https://azure.github.io/Azure-Landing-Zones-Library/) for more information on how to create your own custom library and additional information but for now we are going to copy the alz folder from the [Azure Landing Zones Library](https://github.com/Azure/Azure-Landing-Zones-Library/tree/main/platform) and place it in the lib folder in our config directory.

By default, the architecture that will be deloyed will be _(you can also refer to the README file in the lib folder for more information)_:
...
:::tip
Not required, but make e [alzlibtool](https://github.com/Azure/alzlib) tool, this is a tool that can help you create and manage your own custom library for your Azure Landing Zones, and can be used to check policy and azure landing zone architecture definition structure and document your own custom library.
:::

7. Now that the configuratin files for the bootstrap, Terraform are in place and the architectypes its time to start the bootstrap. Run the following command to start the bootstrap process:

:::tip
If you need to make any last minute changes, after the Terraform plan and before the apply, you can make manual changes to the Terraform code, check your local output folder. Its not something I would rely on - ideally the platform tfvars and yaml input file should be updated, but in a pinch, you can make manual changes. I had to do this recently, when deloying a Landing Zone into New Zealand North, and had tomanually update the location of the Log Analytics workspace, as it was not supported in that region yet. Although for Platform resouces, you can also make this change once the code is in the repository.
:::
...
Once completed, in Azure, you should have 2 resources groups - one for your Managed identities that will be linked to 2 Service Connections in Azure DevOps for Plan and Apply, and one for your Teraform state file.
...
:::info
The plz acronymn I am using, is mean't to indicate 'Platform Landing Zone'.
:::

And Pipelines preconfigured - Continous Interation _(ie Terraform Validate and Plan)_ and Continous Delivery _(ie Terraform Apply)_ with the approvers and branch policies in place.

![Azure DevOps Azure Landing Zone Pipelines](images/ALZ_Bootstrap_ADOPipelines.jpg)

:::tip
I recommend renaming the pipelines, in Azure DevOps so they are more descriptive, ie 'Terraform Plan' and 'Terraform Apply' - pretty simplified, but can make more sense to people consumping the pipelines.

![Azure DevOps Azure Landing Zone Pipelines](images/ALZ_DeployBootstrap_RenameADOPipelines.gif)
:::

Now, lets run the Continous Integration pipeline to see what the Terraform plan looks like.
...
So, now lets deploy. Once you are happy with the plan, you can run the Continous Delivery pipeline to deploy the Platform Landing Zone. 
