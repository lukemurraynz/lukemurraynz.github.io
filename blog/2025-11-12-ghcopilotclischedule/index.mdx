...
Being able to assign [GitHub Coding Agent](https://docs.github.com/en/copilot/concepts/agents/coding-agent/about-coding-agent) to an Issue, is a great way to target a specific task, or issue, but what if you want to run an agent on a schedule? In this post, we will do just that by taking a agent designed to monitors your codebase for performance issues, and then automatically create GitHub issues with any findings - and with [GitHub Copilot CLI](https://docs.github.com/en/copilot/concepts/agents/about-copilot-cli) and GitHub Actions, schedule this to run weekly.

The test agent we will use is `Performance Regression Detector` - this agent is designed to: analyzes your codebase to identify common performance anti-patterns and made purely as a test, for this blog article.

The agent will run weekly (every Wednesday at 9:00 AM UTC) from a GitHub Actions workflow, and creates a GitHub issue with detailed findings, including code locations, severity levels, and recommended fixes.
...
These permissions will be used by GitHub Copilot CLI to create and view existing issues and run the agent with a copilot request.

Once the PAT has been created, then add it to the repository secrets named `COPILOT_GITHUB_TOKEN`, the GitHub Action will then load this as an environment variable.
    ...
    steps:
      ...
      - name: Run Performance Detector Agent
        ...
        env:
          ...
          # CI=true: Signals to tools they're running in CI environment
          # NO_COLOR=1: Disables ANSI color codes in output for cleaner logs
          # TERM=dumb: Indicates terminal doesn't support interactive features
          CI: true
          NO_COLOR: 1
          TERM: dumb
```

> You may have to add to the copilot run step to to `--allow-all-paths` to disable path verification prompts, as this will run in a non-interactive CI environment, depending on your Agent prompt.

## Running the Workflow

Now we can either trigger the Action manually, or wait for it to run on the scheduled time, and after a few minutes..
...
> This article was inspired by [Will Velida](https://www.willvelida.com/) [Cleanup Specialist](https://github.com/willvelida/biotrackr/blob/main/.github/agents/cleanup-specialist.agent.md) agent, which is another great example of a useful Agent, that could be scheduled in a similar way.
