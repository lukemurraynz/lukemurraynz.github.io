...
metaDescription: Learn about the best practices and frameworks for securing container supply chains to protect against vulnerabilities and threats.
...
Today, we are going to take a look inside the [Containers Secure Supply Chain Framework](https://learn.microsoft.com/azure/security/container-secure-supply-chain/articles/container-secure-supply-chain-implementation/containers-secure-supply-chain-overview?WT.mc_id=AZ-MVP-5004796), a framework from Microsoft that utilises an agile ecosystem of tools and processes built to integrate and execute security controls throughout the lifecycle of containers.
...
> Microsoft's [Containers Secure Supply Chain Framework](https://learn.microsoft.com/azure/security/container-secure-supply-chain/articles/container-secure-supply-chain-implementation/containers-secure-supply-chain-overview?WT.mc_id=AZ-MVP-5004796) framework is a seamless, agile ecosystem of tools and processes built to integrate and execute security controls throughout the lifecycle of containers. The container secure supply chain strategy is built considering all the security needs of the container applications. The goal is to prevent the use of vulnerable container images and create container infrastructure with a standard security portfolio.
...
So in essence, the Containers Secure Supply Chain (CSSC) framework exists to ensure the security and integrity of containerized applications throughout their lifecycle. It aims to prevent the use of vulnerable container images and create a secure container infrastructure by integrating security controls at every stage of the supply chain.
...
During each stage, make sure you ensure you have gates in place, to ensure that the image is secure, and meets the minimum quality standard, gets peer-approval, before moving to the next stage. This could be done by using Azure Policy, Azure Security Center, or CI/CD with tools such as Azure DevOps or GitHub.
...
We will look at using seperate Container Registries, one for each stage, to ensure that the security controls are implemented at each stage.
...
:::tip
Make use of [Azure Container Registry webhooks](https://learn.microsoft.com/en-us/azure/container-registry/container-registry-webhook?WT.mc_id=AZ-MVP-5004796), to trigger awareness and automation, when images are pushed to the registries. This could be used to trigger a pipeline, to scan the images, and move them to the next stage, if they meet the minimum quality standard.
:::
...
* It involves centralizing acquisition and management of external artifacts, importing images into an internal registry, and enriching images with metadata like SBOMs and provenance.
* Key tools include Azure Container Registry (ACR) for storing and distributing images, ORAS for interacting with OCI registries, ACR Tasks for automating tasks, and Microsoft Defender for Cloud for security.
...
Lets take a look at [Quarantine](https://learn.microsoft.com/azure/architecture/patterns/quarantine?WT.mc_id=AZ-MVP-5004796) functionity of Azure Container Registry, which could be used to automate the process of scanning and quarantining images that do not meet the minimum quality standard.
...
```
Once the image is quarantined, you will need a user with the AcrQuarantineReader role. The presumption here is the Vulnerability Scanning solution is configured to use this account.

We can run the following Azure CLI, to check the status of the image in the Quarantine.
...
2. You can also make use of [Trivy](https://trivy.dev/), to scan the images for vulnerabilities, and reveal more about the SBOM (Software Bill of Materials), used in the image.
...
Once you have determined the state of your Container Image is good, you can remove the Quarantine flag from your image by running the following Azure CLI command.

In order to call the API, currently you need both Contributor and AcrQuarantineWriter (or AcrPush role) permissions on the registry.
...
In case enterprises are not able to do so, we recommend the following practices for catalog of container images.
...
After the images have been acquired, and vetted by the previous step, and meet organisational requirements, its time to move the images into a Catalog Container Registry, where they can be stored in a central repository, and easily accessed.

Unlike the Acquire Container Registry, this Registry will not be quarantined, however, this registry will need to be continously scanned for vulnerabilities, and malware, as part of operations, using tools like Defender for Cloud and Trivy.
...
It can also be valid, to move images from the Catalog, back to the Quarantine registry, if the Catalog registry is not monitored, or the image has been found to have high priroties vulnerabilities, that fall beneath an organisation threshold.

To move an image between the Quarantine and Catalog registries, you can use the following Azure CLI command.
...
Build phase is all about building application images, in a consistant and secure way. 
...
The Deploy stage is the fourth step in the container supply chain. It involves deploying container images to production environments, where they can be accessed by end-users.

![Build](images/ContainerSupplyFramework_Deploy.PNG)

This stage, is usually where you would move the images downstream from the Catalog registry, to the application Deployment registeries, where the images are deployed to production environments.

Use a secure and scalable private registry for application deployments. Ensure that the registry is protected with role-based access control (RBAC) and that images are signed and have the correct metadata attached.

* Continously scan for vulnerabilities and malware
* Keep artifact metadata up-to-date
* Enforce deployment policies

Just because the images have been moved to the Deployment registry, does not mean that the images are safe to deploy. You should still be scanning the images for vulnerabilities, and malware, and ensuring that the images are signed, and have the correct metadata attached.
...
This the stage, where Copacetic can assist, with patching containers that are deployed, and ensuring that the images are up to date, and secure at the OS layers, while the images are fully patched, tested back at the Build and Catalog stages.

## 🚀Stage 5: Runtime

The Runtime stage is the fifth and final step in the container supply chain. It involves monitoring and securing running containers in production environments, where they can be accessed by end-users.

![Run](images/ContainerSupplyFramework_Run.PNG)

This is one of the most important stages, as this is where the images are running in production, and are exposed to the internet. You should be monitoring the images for vulnerabilities, and malware, and ensuring that the images are signed, and have the correct metadata attached.
