...
Today, we will deploy it into an Azure WebApp, using GitHub actions and the Azure Naming Tool API to generate a name for our storage account bicep resource.
...
:::tip
The process is reasonably well documented directly on [Wiki of GitHub repository](https://github.com/mspnp/AzureNamingTool/wiki) for Azure Naming Tool, and it is what I followed, but to trigger the Publishing Profile button (if its greyed out) in the Azure Portal, for a brand new web app, I was able to go into Configuration/General Settings and Turn Basic Auth Publishing Off, Save, then back on, Save, and the button was enabled.
:::
