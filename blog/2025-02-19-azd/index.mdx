...
Getting started with the Azure Developer CLI (azd) means installing the CLI and creating a new project.
...
Now we have a fully deployed application using `and up` — on to environment management.
...
So we have an environment named: testdeploy, and we can create a new environment and switch to it.
...
After switching back to the testdeploy environment:

![azd monitor](images/demo-azdmonitor.gif)

### Destroying resources (azd down)
