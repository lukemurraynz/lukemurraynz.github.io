...
The goal here is not architecture purity. It is to pick a pattern that survives real operations: DNS behavior, health probes, private-link approval flow, certificate lifecycle, and failure domains.
...
- We need secure public ingress with predictable operations.
- We care about clear blast radius when things fail.
...
- Premium v2 supports modern platform capabilities, but if strict APIM multi-region is required today, classic Premium remains the stronger fit.

### Design implication for this architecture
...
- APIM in a tier/topology that matches required multi-region behavior.
...
The hardest part was not APIM policy authoring. It was making ingress topology and private network behavior line up under real conditions.
...
Most 5xx incidents we saw were traceable to probe path/protocol mismatch, host-header mismatch, or TLS name-check mismatch.
...
- AFD WAF gives early filtering at global edge.
...
Front Door Standard/Premium does not support client mTLS at edge.
...
Use certificate-based controls where stronger service-to-service identity is needed.
...
Use **AFD + WAF -> AppGw -> APIM (internal)** as the default production pattern while strict private APIM posture remains a requirement.

Keep APIM as the single API governance control plane, and AppGw as the private ingress bridge.

If requirements change and strict internal APIM is no longer required, re-evaluate to reduce layers and operational overhead.
