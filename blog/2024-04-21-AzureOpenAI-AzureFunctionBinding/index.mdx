...
If you encounter this error, it could be likely that you are using the Azure Function ExtensionBundle does not contain the necessary dependencies to query the OpenAI API. 

The binding requires at least:
...
:::error
The binding type(s) 'textCompletion' were not found in the configured extension buindle. Please ensure the correct version of the extension bundle is configured.
:::

Both of the two packages are not integrated in the [4.13.2](https://github.com/Azure/azure-functions-extension-bundles/releases/tag/4.13.2) version of the ExtensionBundle, which at the time of writing are not avaliable in the production ExtensionBundle shipped wth new Azure Functions *(which is 4.13.2)*. 
...
🔧 To use the latest Preview Bundle, and enable the OpenAI binding extension, you need to update your funtion apps: host.json file:
