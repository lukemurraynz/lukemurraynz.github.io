...
[Streamlit](https://streamlit.io/) is a great tool for creating interactive web applications with Python. 
...
Lets start off with the Streamlit application.
...
The next step is to create a `requirements.txt` file that specifies the dependencies for the Streamlit application. This will be used to install the python dependencies for the image.
...
I used Draft and the DevX extension were used to draft the first version of the streamlit dockerfile.
...
To build this image, I am going to use Docker on a GitHub Codespace, to build the image locally and test it. Once confirmed, we can push it to an Azure Container Registry, which can then be used to deploy the image to Azure Container Apps.

We can easilly test and build the image locally in a Codespace, using the [Docker extension](https://code.visualstudio.com/docs/containers/overview?WT.mc_id=AZ-MVP-5004796) in Visual Studio Code.
...
One of the best features of Azure Container Apps is the ability to scale the application to zero, so we can stand up a Scale rule to scale _(replicas)_ the Container App down to 0 when not in use.
