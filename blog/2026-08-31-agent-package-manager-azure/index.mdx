...
- [Microsoft Learn MCP](https://learn.microsoft.com/training/support/mcp?WT.mc_id=AZ-MVP-5004796) for current Azure documentation.
- [Azure MCP](https://learn.microsoft.com/azure/developer/azure-mcp-server/overview?WT.mc_id=AZ-MVP-5004796) for cloud resource inspection and operations.
- [Foundry MCP](https://learn.microsoft.com/azure/foundry/mcp/get-started?tabs=user&WT.mc_id=AZ-MVP-5004796) for Microsoft Foundry work.
- [Context7](https://context7.com/) for current SDK and framework documentation.
- Team-specific skills for [Azure Kubernetes Service](https://learn.microsoft.com/azure/aks/what-is-aks?WT.mc_id=AZ-MVP-5004796), [App Service](https://learn.microsoft.com/azure/app-service/overview?WT.mc_id=AZ-MVP-5004796), [Azure Functions](https://learn.microsoft.com/azure/azure-functions/functions-overview?WT.mc_id=AZ-MVP-5004796), [Azure API Management](https://learn.microsoft.com/azure/api-management/api-management-key-concepts?WT.mc_id=AZ-MVP-5004796), identity, monitoring, reliability, and cost.

In my own `Coding` repo, the APM manifest now declares the active agent targets explicitly:
...
They need a **common starting point**.
...
Teams still maintain their own `apm.yml`.
...
This is important because APM policy is an **install-time and CI governance mechanism**, not a replacement for runtime security or agent sandboxing. Runtime permissions still belong to the agent harness and the underlying platform. [APM policy documentation](https://microsoft.github.io/apm/enterprise/apm-policy-getting-started/?utm_source=chatgpt.com)
...
The goal is to make the **supported path easier than building an agent environment from scratch**.
...
[Hypervelocity Engineering (HVE) Core](https://github.com/microsoft/hve-core) is Microsoft's open-source library of agentic SDLC workflow building blocks for GitHub Copilot. It brings together specialised agents for research, planning, implementation, and review; reusable prompts; coding instructions; and skills.
...
The following is a complete example for someone new to HVE Core: declare the exact source, allow it in policy, install it, and commit the resulting lockfile. The installed content lands in the shared `.agents/skills` directory, which supported agent harnesses can use.
...
```yaml title="apm-policy.yml"
dependencies:
  allow:
    - microsoft/hve-core
```

```powershell title="Install and verify"
apm install --target agent-skills
apm audit --ci --policy .\apm-policy.yml
```

This recorded run uses that setup in an isolated `agent-skills` target. APM resolves the exact commit, integrates 74 HVE Core skills, and records the commit and content hash in its lockfile:

![APM installing the pinned HVE Core plugin, integrating 74 skills, and recording its lockfile entry](./images/apm-hve-core-native.gif)
...
- [Hypervelocity Engineering Core](https://github.com/microsoft/hve-core)
