...
Similar to [traceparent](https://www.w3.org/TR/trace-context/#traceparent-header-field-values), the Correlation ID is a unique identifier that is passed along with the request and response headers. It allows you to track the flow of a request as it moves through different services and systems.

In this article, we will explore how to generate a new correlation id through API Management Inbound policies, and pass it back to the Client through an outbound policy, which then then be used to help troubleshoot specific calls with [Application Insights](https://learn.microsoft.com/azure/azure-monitor/app/app-insights-overview?WT.mc_id=AZ-MVP-5004796).

{/* truncate */}

So if want an experience like the following, to help pass the Correlation ID back to the client, then look up the transaction in Application Insights then lets take a look:
...
Now, we need to set an Inbound Policy to create the Correlation ID (if its not already passed in the request) as a variable, then assign that variable to an inbound header.
...
| `<value>@((string)context.Variables["correlation-id"])</value>` | This line is setting the value of the "x-correlation-id" header to the value of the "correlation-id" variable. |
| `</set-header>` | This line is closing the `<set-header>` tag. |

Once thats added, we need to add another inbound policy:
...
This will help you track the Correlation ID in the logs, and help you troubleshoot any issues that may arise.

Once that has been added, its time to output it back to the client. This can be done with the following Outbound policy:
