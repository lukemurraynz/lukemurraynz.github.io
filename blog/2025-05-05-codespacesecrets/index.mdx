...
This is a root level configuration, so its at the same level as features.
...
This is a root level configuration, just like Features and customizations so sits at the same level, like below:.
...
This actually allows you to prompt for the secrets as part of the Codespace build.

![Codespace Secret Configuration Test](images/GitHubCodespace_CodespaceSettingTest.gif)

And is accessed normally through the environment variables as well:

![Codespace Secret Configuration Test](images/GitHubCodespace_CodespaceSettingTestEnv.gif)

For those with the keen eye, will also realise that the environment variables are now entered into your Secrets Account settings scoped to your repository:
...
:::warning
These are not encrypted, even though they are scoped to your account only if you output these in text, it will be shown so be careful with their use in your application.
:::
